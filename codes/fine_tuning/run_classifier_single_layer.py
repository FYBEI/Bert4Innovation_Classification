...
from codes.fine_tuning import tokenization
from codes.fine_tuning.modeling_single_layer import BertConfig, BertForSequenceClassification
from codes.fine_tuning.optimization import BERTAdam
...
class InnovationProcessor(DataProcessor):
    """Processor for the Innovation data set"""
...
class InnovationProcessor_sep(DataProcessor):
    """Processor for the Innovation data set"""
    ...
    def get_dev_examples(self, data_dir):
        train_data = pd.read_csv(os.path.join(data_dir, 'test.csv'), header=None).values
        return self._create_examples(train_data, "dev")
...
def main():
    ...
    parser.add_argument("--discr",
                        default=False,
                        action='store_true',
                        help="Whether to do discriminative fine_tuning.")
    ...
    parser.add_argument('--export_dir',
                        type=str,
                        default=None,
                        help="The dir where to exported the model will be written")
    args = parser.parse_args()

    processors = {
        "innovation": InnovationProcessor,
        "innovation_sep": InnovationProcessor_sep,
        "ag": AGNewsProcessor,
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            model.eval()
            eval_loss, eval_accuracy, eval_recall, eval_pre, eval_f1 = 0, 0, 0, 0, 0
            ...
            with open(os.path.join(args.output_dir, "results_ep"+str(epoch)+".txt"),"w") as f:
                for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluate"):
                    ...
                    for i in range(len(label_ids)):
                        if label_ids[i] == 0 and outputs[i] == 0:
                            tp += 1
                        elif label_ids[i] == 1 and outputs[i] == 1:
                            tn += 1
                        elif label_ids[i] == 0 and outputs[i] == 1:
                            fn += 1
                        elif label_ids[i] == 1 and outputs[i] == 0:
                            fp += 1
            ...
            if tp != 0:
                ...
                eval_f1 = 2 * eval_pre * eval_recall / (eval_recall + eval_pre)
            else:
                eval_pre = 0
                eval_recall = 0
                eval_f1 = 0
    ...
    if args.do_export:
        if os.path.exists(args.export_dir) and os.listdir(args.export_dir):
            raise ValueError("Export directory ({}) already exists and is not empty.".format(args.export_dir))
        os.makedirs(args.export_dir, exist_ok=True)

        save_path = os.path.join(args.export_dir, 'model.pkl')
